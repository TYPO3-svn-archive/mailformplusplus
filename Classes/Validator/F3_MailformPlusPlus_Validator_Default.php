...
class F3_MailformPlusPlus_Validator_Default extends F3_MailformPlusPlus_AbstractValidator {
					...
					foreach($errorChecks as $check) {
						switch($check['check']) {
							...
							case "eregi":
								...
								//value should match a perl regular expression
							case "pregMatch":
								$checkFailed = $this->validatePregMatch($check,$name);
								break;
	 ...
	 * @return string The error string
	 */
	protected function validatePregMatch(&$check,$name) {
		$checkFailed = "";
		$regex = $check['params']['value'];
		if($regex && !preg_match($regex, $this->gp[$name])) {
			$checkFailed = $this->getCheckFailed($check);
		}
		return $checkFailed;
	}
	 ...
	 * @return string The error string
	 */
	protected function validateCalculatingCaptcha(&$check,$name) {
